...
class BooksOrder extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8081/lms/viewOrderList")
      .then((response) => {
  ...
  handleDelete = (id) => {
    axios.delete(`http://localhost:8081/lms/cancelOrder/${id}`).then((res) => {
  ...
  render() {
    return (
      <div className = "container">
        <Link to="/lms/viewOrderList/add" className="btn btn-secondary btn-large mt-3 float-end">
          Add
        </Link>
        <h1>Order Page</h1>
