...
const AddReader = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    if (errors) return;
    axios
      .post("http://localhost:8081/api/register", reader)
      .then((res) => props.history.push("/readers"))
      .catch((err) => {
  ...
  return (
    <div>
      <Typography variant="h3">Reader section</Typography>
      <Grid container>
        <Grid item xs={4} style={{ marginLeft: "auto", marginRight: "auto" }}>
          ...
          >
            ...
            <Box mb={3}>
              ...
              />
              {errors && (
                <Typography variant="caption">{errors.passwrod}</Typography>
              )}
            </Box>
