...
class DamagedBooks extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8081/lms/viewDamagedBooksList")
      .then((response) => {
  ...
  handleDelete = (id) => {
    axios.delete(`http://localhost:8081/lms/deleteDamagedBooks/${id}`).then((res) => {
  ...
  render() {
    return (
      <div className = "container">
        <Link to="/lms/viewDamagedBooksList/add" className="btn btn-secondary btn-large mt-3 float-end">
          Add
        </Link>
