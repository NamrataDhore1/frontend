...
class BooksReturn extends React.Component {
  ...
  componentDidMount() {
    axios
      .get("http://localhost:8081/lms/viewbooklist")
      .then((response) => {
  ...
  handleDelete = (id) => {
    axios.delete(`http://localhost:8081/lms/deleteReturnBookDetails/${id}`).then((res) => {
      const  booksreturned = this.state. booksreturned.filter((au) => au.id != id);
  ...
  render() {
    return (
      <div className = "container">
        <Link to="/lms/viewbooklist/add" className="btn btn-secondary btn-large mt-3 float-end">
          Add
        </Link>
        <h1>Books Page</h1>
